...
## FLOWのビジョン
FLOWは「データ × 時間軸 × 位置 → 未来の需要を予測して先回りする」タクシー需要予測アプリ。
全ての機能はこの軸に沿って設計すること。

### 開発ステージ
- Stage 1（現在）: イベント・ホテル・ロングスポットのリアルタイムデータで「これから起きる需要」を予測
- Stage 2（次）: 過去の売上データ（taxi-sales-log）と組み合わせて「曜日×時間帯×場所」の需要パターンを学習
- Stage 3（将来）: 天気・季節・特殊イベントなど外部データを加えた総合予測

### 判断基準
新機能を追加する時は「これは未来の需要予測に貢献するか？」で判断する。
見た目のカッコよさより、運転中にチラ見で意思決定できる実用性を最優先。
...
## バグ修正のルール
- バグ修正はいきなりコードを変更しない
- 必ず先にコードを読んで原因を特定し、修正計画を報告してから実行すること
- 特にCSS/レイアウト系の問題は、該当するCSSルールを全て洗い出してから修正する
